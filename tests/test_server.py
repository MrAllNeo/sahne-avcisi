...
def running_server(env_overrides: dict[str, str]):
    ...
    try:
        ...
        temp_dir.cleanup()
        # The reload above rebinds the module-level APP to a database inside the
        # temp directory we just deleted. Reload once more with the real
        # environment so later tests do not inherit a dead handle.
        importlib.reload(server_module)
